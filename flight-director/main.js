...
function showSecondScreen(e) {
  ...
  document
    .querySelectorAll('#time-screen #time-buttons .time-button')
    .forEach((el) => {
      el.addEventListener('click', createInterface);
    });
}

function createInterface(e) {
  totalSec = Number(e.target.getAttribute('data-time'));
  document
    .querySelectorAll('#time-screen #time-buttons .time-button')
    .forEach((el) => {
      el.removeEventListener('click', createInterface);
    });
  document.getElementById('tsiderx').innerHTML = String('Score: ' + 0);
  showScreen('#test');
  gameCont = new createjs.Container();
  // gameCont.regX = stage.canvas.width / 2;
...
function startMain() {
  ...
  timeCounter = window.setInterval(countTimeMain, 1000);
  questionColor();
  activeAudio();
}
...
function activeAudio() {
  ...
  instance.on('complete', createjs.proxy(this.handleCompleteSoundActive, this));
  instance.volume = 0;
}
function handleCompleteSoundActive() {
    return;
}
...
function showEndScreen() {
  ...
  document.querySelector('#tsider').innerHTML = String('End of Exam');
...
function startAgain(e) {
  ...
  document.getElementById('tsiderx').innerHTML = '- / -';
  showStartScreen();
}
//save the score
